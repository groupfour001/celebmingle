...
function updatePaymentInstructions() {
    ...
    const cryptoAmount = (orderAmount * crypto.ratePerUSD).toFixed(crypto.symbol === 'USDT' ? 2 : 8);
    
    // Update rate displays
    document.getElementById(`${selectedCrypto}Rate`).textContent = 
        crypto.symbol === 'USDT' ? `${cryptoAmount} ${crypto.symbol}` : `≈ ${cryptoAmount} ${crypto.symbol}`;
...
// Update crypto rates for all options using live rates
async function updateCryptoRates() {
    const rates = await (typeof fetchLiveRates === 'function' ? fetchLiveRates() : Promise.resolve({btc:24000,eth:3000,usdt:1}));
    const usdAmount = orderAmount || 299.99;
    const cryptoData = {
        btc: { amount: (usdAmount / rates.btc).toFixed(8), symbol: 'BTC', rate: rates.btc },
        eth: { amount: (usdAmount / rates.eth).toFixed(8), symbol: 'ETH', rate: rates.eth },
        usdt: { amount: usdAmount.toFixed(2), symbol: 'USDT', rate: 1 }
    };
    Object.keys(cryptoData).forEach(key => {
        const rateElement = document.getElementById(`${key}Rate`);
        if (rateElement) {
            rateElement.textContent = key === 'usdt' ? `${cryptoData[key].amount} USDT` : `≈ ${cryptoData[key].amount} ${cryptoData[key].symbol}`;
        }
        // Add data-rate attribute for BTC and ETH
        if (key === 'btc' || key === 'eth') {
            const card = document.getElementById(`${key}Card`);
            if (card) card.setAttribute('data-rate', cryptoData[key].rate);
        }
    });
}
...
function handleSubmitPayment() {
    ...
    setTimeout(() => {
        ...
        // Show centered message overlay
        let centerMsg = document.createElement('div');
        centerMsg.id = 'centerDetailsMsg';
        ...
        `;
        centerMsg.innerHTML = `
            <div style="background: #fffbe6; border-radius: 18px; box-shadow: 0 4px 32px #000a; padding: 2.5rem 2.8rem; text-align: center; max-width: 370px; margin: auto;">
        ...
        setTimeout(function() {
            let btn = document.getElementById('returnHomeBtn');
            if (btn) {
