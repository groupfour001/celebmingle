document.addEventListener('DOMContentLoaded', () => {
    // Set Firebase Auth persistence to LOCAL (keeps user logged in across sessions)
    ...
    // Membership link access control
    function showMembershipLoginMessage() {
        const oldMsg = document.getElementById('membership-login-message');
        if (oldMsg) oldMsg.remove();
        const msg = document.createElement('div');
        msg.id = 'membership-login-message';
        ...
        msg.innerHTML = '<div style="background: #fff; padding: 40px 32px; border-radius: 18px; box-shadow: 0 8px 32px rgba(44,62,80,0.18); text-align: center; font-size: 1.3rem; color: #232a34; font-family: Montserrat, sans-serif; min-width: 260px; max-width: 90vw;">You must login to access this feature<br><button id="close-membership-login-message" style="margin-top: 24px; background: linear-gradient(90deg, #FFD700, #FFA500); color: #232a34; border: none; border-radius: 12px; padding: 10px 28px; font-size: 1.1rem; font-weight: 700; cursor: pointer;">Close</button></div>';
        document.body.appendChild(msg);
        document.getElementById('close-membership-login-message').onclick = function () {
            msg.remove();
        };
    }

    function updateMembershipLink() {
        const membLink = document.getElementById('membuser');
        ...
        if (membLink) {
            if (isLoggedIn) {
                membLink.style.pointerEvents = 'auto';
                membLink.style.opacity = '1';
            } else {
                membLink.style.pointerEvents = 'auto'; // allow click to show message
                membLink.style.opacity = '1';
            }
        }
    }
    ...
    document.addEventListener('click', function(e) {
        ...
        if (membLink && e.target === membLink) {
            ...
            if (!isLoggedIn) {
                e.preventDefault();
                showMembershipLoginMessage();
            }
        }
    });

    // Call on load and on auth change
...
function handleSignOut() {
    ...
    // Clear auth state ONLY on explicit logout
    localStorage.removeItem('authState');
    localStorage.removeItem('userInfo');
    localStorage.removeItem('userEmail');
